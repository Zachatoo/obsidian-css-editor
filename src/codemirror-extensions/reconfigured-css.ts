...
function isSelectorIdentifier(node: SyntaxNodeRef) {
	if (node.name == "ClassName") return SelectorType.CLASS;
	if (node.name == "IdName") return SelectorType.ID;
	return false;
}
...
function touchSelection(
	...
	return (
		selection.ranges.findIndex(
			(selRange) =>
				selRange.from <= range.to && selRange.to >= range.from,
		) >= 0
	);
}
...
function getSelectorNames(
	...
	if (node.to - node.from > 4096) {
		...
		node.cursor().iterate((node) => {
			if (
				type == isSelectorIdentifier(node) &&
				!touchSelection(selection, node)
			) {
